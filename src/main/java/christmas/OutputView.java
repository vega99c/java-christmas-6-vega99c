...
import christmas.domain.event.Badge.BadgeType;
...
public class OutputView {
    ...
    private final String TOTAL_PRICE_BEFORE_DISCOUNT_MESSAGE = "<할인 전 총주문 금액>";
    private final String GIVEN_NOTHING = "없음";
    private final String GIFTS_MESSAGE = "<증정 메뉴>";
    private final String BENEFITS_HISTORY_MESSAGE = "<혜택 내역>";
    private final String TOTAL_BENEFITS_MESSAGE = "<총혜택 금액>";
    private final String EVENT_BADGE_MESSAGE = "<%d월 이벤트 배지>";
    private final String TOTAL_PRICE_AFTER_DISCOUNT_MESSAGE = "<할인 후 예상 결제 금액>";
    ...
    public void printCustomerOrders(List<OrderCompletedMenu> orders) {
        System.out.print(ORDER_MENU_MESSAGE + LINE_SEPARATOR);
        for (OrderCompletedMenu menu : orders) {
            System.out.printf("%s %d개" + LINE_SEPARATOR, menu.getMenu().getMenuName(), menu.getQuantity());
        }
        
        System.out.println();
    }

    public void printEventPreviewMessage(int month, int day) {
    ...
    public void isNoting() {
        System.out.print(BENEFITS_HISTORY_MESSAGE + LINE_SEPARATOR + GIVEN_NOTHING + DOUBLE_LINE_SEPARATOR);
    }

    public void printNoticeBenefitsHistory() {
        System.out.print(BENEFITS_HISTORY_MESSAGE + LINE_SEPARATOR);
    }
    ...
    public void printTotalBenefits(int totalBenefits) {
        System.out.print(TOTAL_BENEFITS_MESSAGE + LINE_SEPARATOR);
        if (totalBenefits != 0) {
    ...
    public void printTotalPriceAfterDiscount(int totalPrice) {
        System.out.print(TOTAL_PRICE_AFTER_DISCOUNT_MESSAGE + LINE_SEPARATOR);
        System.out.printf("%s원" + DOUBLE_LINE_SEPARATOR, moneyFormat.format(totalPrice));
    }

    public void printEventBadge(int eventMonth, BadgeType badge) {
        System.out.printf(EVENT_BADGE_MESSAGE + LINE_SEPARATOR + badge.getBadgeName(), eventMonth);
    }
}
